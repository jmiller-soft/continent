package com.continent.engine.skein;
    ...
    implements Memoable
{
     ...
     *                       bytes.
     */
    public SkeinEngine(int blockSizeBits, int outputSizeBits, long rounds)
    {
        ...
        this.rounds = rounds;
        this.threefish = new ThreefishEngine(blockSizeBits, rounds);
        this.ubi = new SkeinEngine.UBI(threefish.getBlockSize());
    }
    ...
    {
        this(engine.getBlockSize() * 8, engine.getOutputSize() * 8, engine.rounds);
        copyIn(engine);
    }
    ...
    private void initParams(Hashtable parameters)
    {
        ...
        while (keys.hasMoreElements())
        {
            ...
            {
                pre.addElement(new SkeinEngine.Parameter(type.intValue(), value));
            }
            else
            {
                post.addElement(new SkeinEngine.Parameter(type.intValue(), value));
            }
        }
        preMessageParameters = new SkeinEngine.Parameter[pre.size()];
        pre.copyInto(preMessageParameters);
        sort(preMessageParameters);

        postMessageParameters = new SkeinEngine.Parameter[post.size()];
        post.copyInto(postMessageParameters);
        sort(postMessageParameters);
    }

    private static void sort(SkeinEngine.Parameter[] params)
    {
        if (params == null)
        {
            return;
        ...
        {
            SkeinEngine.Parameter param = params[i];
            int hole = i;
    ...
    private void createInitialState()
    {
        ...
        else
        {
            ...
            ubiComplete(PARAM_TYPE_CONFIG, new SkeinEngine.Configuration(outputSizeBytes * 8).getBytes());
        }
        ...
        if (preMessageParameters != null)
        {
            ...
            {
                SkeinEngine.Parameter param = preMessageParameters[i];
    ...
    public int doFinal(byte[] out, int outOff)
    {
        ...
        if (postMessageParameters != null)
        {
            ...
            {
                SkeinEngine.Parameter param = postMessageParameters[i];
                ubiComplete(param.getType(), param.getValue());
            }
        }
